...
class NeuralNetwork {
  ...
  void train(Matrix inp, Matrix tgt) {
    ...
    Matrix lhdot1 = output_error.arrayTimes(out[2].arrayTimes(Matrix.identity(3,1).minus(out[2])));
    ...
    Matrix lhdot3 = hidden_error.arrayTimes(out[1].arrayTimes(Matrix.identity(3,1).minus(out[1])));
